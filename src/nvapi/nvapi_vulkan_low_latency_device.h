...
namespace dxvk {
    ...
    enum LowLatencyDeviceType : uint8_t {
        LowLatency2,
        VkReflexFake
    };

    struct NvapiVulkanLowLatencyDevice {
        NvapiVulkanLowLatencyDevice(LowLatencyDeviceType type) : m_type(type) {}
        [[nodiscard]] LowLatencyDeviceType GetType() const { return m_type; }

      private:
        LowLatencyDeviceType m_type;
    };
...
#define PFN_PARAM(proc) PFN_##proc proc
        [[nodiscard]] NvapiVulkanLowLatency2LayerDevice(
            VkDevice device,
